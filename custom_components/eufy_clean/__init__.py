    ...
    async def handle_clean_rooms(call: ServiceCall) -> None:
        ...
        if isinstance(room_ids_str, str):
            try:
                ...
                room_ids = [int(x.strip()) for x in room_ids_str.split(",") if x.strip()]
        else:
            ...
            for entry_id, coordinator in hass.data[DOMAIN].items():
                if isinstance(coordinator, EufyCleanDataUpdateCoordinator):
                    ...
                    for device_id, device in coordinator.devices.items():
                        if entity_entry.unique_id == device_id or entity_entry.unique_id.startswith(device_id):
                            await device.clean_rooms(room_ids, clean_times)
                            _LOGGER.info("Started room cleaning for %s: rooms=%s", entity_id, room_ids)
                            return
